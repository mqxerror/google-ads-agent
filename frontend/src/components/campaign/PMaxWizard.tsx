...
export default function PMaxWizard({ onClose, onBackToTypePicker }: PMaxWizardProps) {
    ...
    switch (stepId) {
      ...
      case 'images':
        return bundle.logos.length >= rules.logos.min
          ...
          && (bundle.landscape.length + bundle.square.length + bundle.portrait.length) <= rules.imageMax;
      case 'videos':
        return true; // optional — video is auto-generated by Google when absent
      case 'signals': {
        if (!rules.searchThemes) return true;
        const themes = bundle.audienceSignals.map(s => s.trim()).filter(Boolean);
        return themes.length <= rules.searchThemes.maxCount
          && themes.every(t => t.length <= rules.searchThemes!.maxChars);
      }
      case 'review':
  ...
  // FR1.4/FR1.3 — every server rejection mirrored client-side (no 422 surprise);
  // soft rules surface as non-blocking warnings.
  const clientIssues = useMemo(() => validatePMax({
    businessName: bundle.businessName,
    finalUrl: bundle.finalUrl,
    headlines: bundle.headlines,
    longHeadlines: bundle.longHeadlines,
    descriptions: bundle.descriptions,
    logos: bundle.logos,
    landscape: bundle.landscape,
    square: bundle.square,
    portrait: bundle.portrait,
    audienceSignals: bundle.audienceSignals,
  }, rules), [bundle, rules]);
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-6">
      ...
      {/* FR1.4/FR1.3 — mirror every server rejection here so no limit is first
          learned from a 422; soft (verified:false) rules render as warnings. */}
      {rules.ready && (clientIssues.errors.length > 0 || clientIssues.warnings.length > 0) && (
        <div className="space-y-1">
          {clientIssues.errors.map((e, i) => (
            <p key={`e${i}`} className="text-[11px] text-destructive flex items-center gap-1.5">
              <AlertCircle className="h-3 w-3 shrink-0" /> {e}
            </p>
          ))}
          {clientIssues.warnings.map((w, i) => (
            <p key={`w${i}`} className="text-[11px] text-warning flex items-center gap-1.5">
  ...
  return (
    <div className="space-y-5">
      ...
      <ImageGroup
        ...
        onAssetKnown={registerAsset}
      />
      {/* Cross-slot meter (honesty #8): Google caps TOTAL images per asset group
          across every ratio, not per slot — surface the running sum + cap. */}
      {(() => {
        const total = bundle.landscape.length + bundle.square.length + bundle.portrait.length;
        const over = total > rules.imageMax;
        return (
          <p className={cn('text-[11px] flex items-center gap-1.5',
            over ? 'text-destructive' : 'text-muted-foreground')}>
            {over && <AlertCircle className="h-3 w-3 shrink-0" />}
            {total}{rules.ready ? `/${rules.imageMax}` : ''} images across ratios
            {over ? ' — over the per-asset-group cap' : ''}
          </p>
        );
      })()}
    </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <TextList
        ...
        // Search-theme limits come from the registry (≤max_chars × ≤max_count) —
        // no baked constant; Infinity = permissive before specs load.
        maxChars={rules.searchThemes?.maxChars ?? Infinity}
        minItems={0}
        maxItems={rules.searchThemes?.maxCount ?? Infinity}
      />
    </div>
  );
}
