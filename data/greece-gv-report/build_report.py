...
Greece Golden Visa - Oman + Jordan Google Search campaign plan.

Reads the real keyword data from greece-gv-kwp-processed.json and emits
report.html with NO external JS, NO CDN, system fonts only, print CSS for A4.

Greece note: RESIDENCY by investment, NOT citizenship. No citizenship /
second-passport framing anywhere. Design system identical to the Panama report.
"""
import json
import html
import os
...
DATA_JSON = os.path.join(BASE, "..", "greece-gv-kwp-processed.json")
...
MONEY_KEYWORDS = {r["keyword"] for r in ROWS if "money term" in r["priority"]}
# keywords that returned real CPC bid data -> highlight
CPC_KEYWORDS = set(DATA["cpc_summary"]["keywords_with_bid_data"])
CPC_LO = DATA["cpc_summary"]["low_top_of_page_usd"]
CPC_HI = DATA["cpc_summary"]["high_top_of_page_usd"]

EN_COUNT = sum(1 for r in ROWS if r["lang"] == "EN")
AR_COUNT = sum(1 for r in ROWS if r["lang"] == "AR")
...
def comp_cell(comp):
    ...
    cls = {"Low": "comp-low", "Medium": "comp-med", "High": "comp-high"}.get(comp, "")
...
def build_keyword_rows():
    """All keywords, sorted volume-desc (vol_max), stable by keyword."""
    ordered = sorted(ROWS, key=lambda r: (-r["vol_max"], r["keyword"]))
    out = []
    for i, r in enumerate(ordered, start=1):
        ...
        lang_badge = (
            '<span class="lang lang-ar">AR</span>' if is_ar
            else '<span class="lang lang-en">EN</span>'
        )
        money_badge = ' <span class="badge-money">GREECE</span>' if is_money else ""
...
# ---------------------------------------------------------------------------
# CSS — identical design system to the Panama report
# ---------------------------------------------------------------------------
CSS = r"""
:root{
  --navy:#0B1F3A;
...
<title>Greece Golden Visa &middot; Oman + Jordan &middot; Google Search Campaign Plan</title>
<style>{CSS}</style>
</head>
<body>
<div class="wrap">
  ...
  <header class="cover">
    ...
    <h1>Greece Golden Visa</h1>
    <div class="sub">Oman&nbsp;+&nbsp;Jordan &middot; Google Search &middot; English&nbsp;+&nbsp;Arabic</div>
    <div class="rule"></div>
    <div class="meta">
      <div>Program<b>Greece Golden Visa &mdash; residence-by-investment</b></div>
    ...
    <div class="prep">
      Prepared by <b>Mercan Group</b> &mdash; an experienced immigration-by-investment advisor.
      Greece grants a <b>renewable 5-year residence permit</b> for a qualifying real-estate
      investment, with Schengen travel. Landing page: www.mercan.com/lp/greece-golden-visa
    </div>
  </header>
  ...
  <section>
    ...
    <p>There is real, capturable demand in Oman and Jordan for an <strong>EU residence-by-investment
    &mdash; a foothold in Europe with Schengen mobility and a stable family base</strong> &mdash; two
    under-served Gulf and Levant markets with high per-capita wealth and strong mobility motives. This
    campaign puts Mercan&rsquo;s Greece Golden Visa in front of that audience on Google Search, in both
    English and Arabic. The program is <strong>residency, not citizenship</strong> &mdash; a renewable
    5-year Greek residence permit earned through a qualifying real-estate investment.</p>

    <h3>The honest reality, stated up front</h3>
    <ul class="tight">
      <li><strong>Total addressable demand is roughly {TOTALS['grand']:,} searches/month</strong> across
      every keyword researched, both countries, both languages (deduplicated). That is a small, premium
      niche &mdash; not a high-volume market.</li>
      <li><strong>Greece-named terms carry real, measurable demand &mdash; a genuine advantage.</strong>
      Unlike thinner destination programs, terms like <em>golden visa greece</em> / <em>greek golden
      visa</em> return <strong>real volume (10&ndash;20/mo) and even CPC bid data</strong> in these
      markets &mdash; Greece has authentic brand-search demand. Whoever types them is already
      considering Greece.</li>
      <li><strong>The strategy splits reach from intent.</strong> Broad head terms
      (<em>golden visa</em>, <em>residency by investment</em>, <em>eu golden visa</em>, and the Arabic
      <span class="rtl-inline">الإقامة الذهبية</span> / <span class="rtl-inline">الفيزا الذهبية</span>)
      carry the <strong>reach</strong>; Greece-named + property terms carry the <strong>intent</strong>.
      Bid to capture both.</li>
      ...
      <li><strong>The lever is a genuine rising-cost reality &mdash; not citizenship.</strong>
      Greece&rsquo;s real-estate thresholds have already been raised and tiered
      (<strong>&euro;250K</strong> conversion projects &middot; <strong>&euro;400K</strong> most of
      Greece &middot; <strong>&euro;800K</strong> prime zones). The honest hook is &ldquo;act while the
      lower-entry routes remain,&rdquo; framed as a residence permit &mdash; never as a passport or
      citizenship.</li>
    </ul>

    <h3>Market shape at a glance</h3>
    <div class="stats">
      {stat_block(f"{TOTALS['EN_total']}", "English / mo", "Where ready-to-invest intent + reach concentrate")}
      {stat_block(f"{TOTALS['AR_total']}", "Arabic / mo", "Real, broader/earlier demand; often cheaper CPCs")}
      {stat_block(f"{TOTALS['Oman_total']}", "Oman / mo", "Near-even with Jordan")}
      {stat_block(f"{TOTALS['Jordan_total']}", "Jordan / mo", "Slightly heavier; strong EU-base motive")}
      {stat_block(f"~{TOTALS['grand']:,}", "Total / mo", "Full addressable market, both langs, both countries")}
    </div>
  ...
  <section class="page-break">
    ...
    cluster, so every ad speaks to exactly the searcher&rsquo;s mindset. Because Greece is a
    <strong>real-estate-driven residency</strong>, the Greece-named and property clusters
    (AG1&nbsp;/&nbsp;AG3&nbsp;/&nbsp;AG5) carry the sharpest intent &mdash; bid up, use [exact],
    write the sharpest ads there.</p>
    <table class="tbl-rec">
      <thead><tr><th>Ad group</th><th>Cluster</th><th>Priority</th><th>Note</th></tr></thead>
      <tbody>
        <tr><td><b>AG1 &middot; Greece Golden Visa</b></td><td>A</td><td><b>Highest intent</b></td><td>Greece-named head terms &mdash; real volume + CPC data; <b>bid up</b>, [exact]</td></tr>
        ...
        <tr><td><b>AG3 &middot; Property / Real-estate Visa</b></td><td>C</td><td><b>Highest intent</b></td><td>The program mechanism: buy property &rarr; residency. Sharp, ready buyers</td></tr>
        <tr><td><b>AG4 &middot; EU Residency / Schengen</b></td><td>D</td><td>High emotion</td><td><b>Residency-accurate only</b> &mdash; &ldquo;EU foothold + Schengen travel,&rdquo; <em>never</em> second passport or citizenship</td></tr>
        <tr><td><b>AG5 &middot; Investment Migration / Investor Visa</b></td><td>E</td><td>High intent</td><td>Greece-named investor-visa terms; ready-to-move-money searchers</td></tr>
        <tr><td><b>AG6 &middot; Comparison / Best-Cheapest-Fastest</b></td><td>F</td><td>Commercial shopper</td><td>Lead with tiered entry (&euro;250K route), Schengen access, EU residency</td></tr>
      </tbody>
    </table>
    <div class="callout note">
      <div class="ct">AG4 accuracy note &mdash; residency, not citizenship</div>
      The Greece Golden Visa is a <strong>renewable residence permit</strong>, <strong>not</strong>
      citizenship and <strong>not</strong> a second passport. Greek citizenship is a separate
      naturalisation route (typically ~7 years of residence). All AG4 copy frames the benefit as
      &ldquo;EU residency with visa-free Schengen travel&rdquo; &mdash; the Arabic states it plainly as
      <span class="rtl-inline">تصريح إقامة (وليس جنسية)</span> (&ldquo;a residence permit, not
      citizenship&rdquo;).
    </div>
    ...
    <ul class="tight">
      ...
      request / guide download / tap-to-call). Confirm the Greece landing page fires <strong>its own
      ...
      <li><strong>AG1 / AG3 / AG5 (Greece-named):</strong> bias bids <strong>up</strong> &mdash; these
      are the readiest buyers and they carry real volume + CPC data. Cap the broad <em>golden visa</em>
      head (Oman high-top-of-page hits <strong>$11.90</strong>) so it doesn&rsquo;t drain the budget.</li>
    </ul>
    ...
    adjacent-but-worthless traffic that never invests six figures &mdash; Greece especially needs a
    strong <em>holiday/tourism</em> block:</p>
    <div class="neg-grid">
      ...
      <div><b>Tourism / holiday</b> &mdash; santorini, mykonos, greek islands tour, greece holiday, greece vacation, hotel booking, flights &middot; <span class="rtl-inline">سياحة، فندق، سانتوريني، ميكونوس</span></div>
      <div><b>Study</b> &mdash; student, study in greece, scholarship, university, erasmus &middot; <span class="rtl-inline">دراسة، منحة</span></div>
      <div><b>Rent / lifestyle</b> &mdash; rent, apartment for rent, cost of living, cheap living, digital nomad &middot; <span class="rtl-inline">إيجار، تكلفة المعيشة، رخيص للسكن</span></div>
      ...
      <div><b>Citizenship mismatch</b> &mdash; greek citizenship, greek passport, greece citizenship by descent (the GV is residency &mdash; these mislead)</div>
      <div><b>Stray</b> &mdash; map, weather, greek language course, translation &middot; <span class="rtl-inline">ترجمة</span></div>
    </div>
    <div class="callout">
      <div class="ct">Ongoing hygiene</div>
      Run the <strong>Search Terms report twice weekly</strong> for the first month and harvest
      negatives aggressively &mdash; a small budget cannot absorb waste. Watch for holiday/tourism
      spillover (Greece is a top holiday destination) and other-country golden-visa searches: decide
      per term whether to exclude or redirect into AG6 (comparison).
    </div>

    <h3>2.5 &nbsp;The rising-cost lever (real &mdash; not a fabricated deadline)</h3>
    <div class="lever">
      <div class="lh">Tiered thresholds &mdash; <span>&euro;250K</span> conversion &middot; <span>&euro;400K</span> standard &middot; <span>&euro;800K</span> prime zones</div>
      <div class="lb">
        Greece has <strong>already raised and tiered</strong> its real-estate thresholds. The honest
        angle is &ldquo;the lower-entry routes are narrowing &mdash; act while they remain,&rdquo; framed
        as a residence permit. <strong>No specific future date is asserted.</strong>
        <ul>
          <li><strong>Sitelink (EN):</strong> &ldquo;Secure the &euro;250K Conversion Route&rdquo; &rarr; deep-link to the LP consultation</li>
          <li><strong>Sitelink (AR):</strong> <span class="rtl-inline">قبل ارتفاع تكاليف الإقامة</span> (&ldquo;before residency costs rise&rdquo;)</li>
          <li><strong>Callout:</strong> &ldquo;EU residency + Schengen travel&rdquo; / &ldquo;Renewable 5-year permit&rdquo; &mdash; never &ldquo;passport&rdquo; or &ldquo;citizenship&rdquo;</li>
        </ul>
      </div>
    </div>
  </section>
  ...
  <section class="page-break">
    ...
    <p>This is a <strong>premium-CPC category</strong> &mdash; golden-visa clicks are among the most
    expensive in paid search. Actual CPC paid usually sits below the top-of-page bid, so we model an
    effective-CPC band:</p>
    ...
    <div class="callout">
      ...
      $30/day answers <em>&ldquo;what does a qualified Greece Golden Visa lead cost in these markets, and
      does English or Arabic convert cheaper?&rdquo;</em> &mdash; not to deliver volume. <strong>Launch
  ...
  <section class="page-break">
    ...
    {TOTALS['Jordan_total']} Jordan, Jordan slightly heavier)</strong>. Exactly why the plan is
    English-first, both-countries, with Arabic layered in as budget scales.</p>

    <h3>4.2 &nbsp;The CPC reality</h3>
    <p>This is a <strong>premium-CPC category.</strong> <strong>{DATA['cpc_summary']['count_keywords_with_bid_data']} keywords</strong>
    returned bid data (the volume-bearing head + Greece-named terms) &mdash; a healthier signal than
    thinner programs where only the generic head term bids. Everything else has too little volume for
    Keyword Planner to report a bid:</p>
    <table class="tbl-rec tbl-num" style="max-width:420px;">
      <thead><tr><th>Bid tier</th><th>Range (USD)</th><th>Average</th></tr></thead>
      <tbody>
        <tr><td><b>Low top-of-page</b></td><td>${CPC_LO['min']:.2f} &ndash; ${CPC_LO['max']:.2f}</td><td><b>${CPC_LO['avg']:.2f}</b></td></tr>
        <tr><td><b>High top-of-page</b></td><td>${CPC_HI['min']:.2f} &ndash; ${CPC_HI['max']:.2f}</td><td><b>${CPC_HI['avg']:.2f}</b></td></tr>
      </tbody>
    </table>
    ...
    worth testing as budget grows. The Greece-named <em>golden visa greece</em> /
    <em>greek golden visa</em> bid at $0.63&ndash;$5.02 &mdash; real, and a sign of authentic
    Greece-specific demand.</p>
    <div class="callout note">
  ...
  <section class="page-break">
    <div class="sec-head"><span class="sec-num">5</span><h2>All {TOTAL_COUNT} researched keywords &mdash; full traffic table</h2></div>
    <p>Every researched keyword ({EN_COUNT} English + {AR_COUNT} Arabic), <strong>sorted highest-volume
    first.</strong> Oman / Jordan columns show Keyword Planner&rsquo;s rounded monthly ranges;
    <span class="mut">&lt;10</span> means below the ~10 searches/month reporting floor. Arabic rows are
    shown right-to-left with transliteration and English gloss beneath. The
    {DATA['cpc_summary']['count_keywords_with_bid_data']} keywords that returned bid data are shown with
    <span class="cpc-hi">highlighted CPC</span>; Greece-named money-terms are
    <span class="badge-money">GREECE</span>-badged and shaded.</p>
    ...
    <div class="legend">
      ...
      <span><span class="sw" style="background:#fbf6ea;border:1px solid #C79A3A"></span>Greece money-term row</span>
...
{KEYWORD_ROWS}
  ...
  <!-- ============ FOOTER ============ -->
  <footer class="foot">
    <b>Prepared by Mercan Group</b> &mdash; experienced immigration-by-investment advisor. The Greece
    Golden Visa is a <b>renewable 5-year residence permit</b> earned through a qualifying real-estate
    investment (residency, not citizenship). This is a precision, high-value-per-lead campaign; success
    is measured in cost-per-qualified-lead, not clicks or impressions.
    <div class="prov-line">
...
print("CPC-bearing keywords:", len(CPC_KEYWORDS))
