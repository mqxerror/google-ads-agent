...
Version: 6 | Last optimized: 2026-04-15 22:23 | Success rate: N/A
...
### Enhanced Conversions (active next priority)
- Enable Google Ads enhanced conversions — match rate improvement is meaningful for HNWI audiences where lead volume is low and value per conversion is high
- If the lead form captures email or phone, hash data client-side before sending to Google if consent flow requires it
- Low lead volume makes every matched conversion count more — enhanced conversions is the highest-leverage next step now that baseline tracking is verified stable
- Do not defer this. Baseline is confirmed. This is the current action item.
...
3. **NEVER flag SEO issues on goldenvisas.mercan.com** — this domain is ADS-ONLY. The `noindex`/`nofollow` tags are INTENTIONAL. Do not flag: indexing, robots.txt, organic traffic, sitemaps, canonical tags, or any organic-signal concern on any page on this domain. This applies to CRO audits, GTM audits, and all other analysis types. Explicitly directed on 2026-04-14.

4. **NEVER use quiz, eligibility check, or lead scoring language** — there is no eligibility gate. Capital is the only qualifier. Any UX copy, funnel step, tracking event name, or CTA must assume the visitor is pre-qualified. This correction was given twice. It is a hard rule.
...
| **Next Priority** | (1) Enhanced conversions setup, (2) attribution window verification — confirm 90-day is active, not assumed |
...
- No outcome cycle completed yet. Tracking layer is stable. Current action items: (1) enhanced conversions setup, (2) confirm 90-day attribution window is active.
