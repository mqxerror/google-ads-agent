...
Version: 6 | Last optimized: 2026-04-15 21:45 | Success rate: N/A
...
### Step 0 — Check Conversion Lag Before Anything Else
This is the single most common misdiagnosis in this account. Before escalating any concern about low conversion volume, high CPA, or apparent tracking failure:
- Confirm: is the data window < 7 days? If yes, conversions are almost certainly still accumulating. Do not act yet.
- Pull a 30-day window and compare recent 7-day conversion pace against the same 7-day slice from the prior 30 days.
- Only after ruling out lag should you investigate campaign-level causes.
...
- When CPA looks high on recent data, verify the attribution window is closed before escalating

### Account-Wide Pattern Recognition — Check Before Campaign-Level Diagnosis
All campaigns share one GTM container, one landing page domain, and one primary conversion event. A problem that looks campaign-specific is often account-wide. Protocol:
1. When one campaign shows anomalous data, immediately check if the pattern appears across all active campaigns.
2. If the pattern is account-wide → root cause is shared infrastructure, shared landing page, or an external market event.
3. Only diagnose a campaign-specific root cause after ruling out account-wide signals.
...
- The correct CTA account-wide is **"Request a Free Consultation"** — applies to all campaigns and all copy recommendations. No alternatives, no exceptions.
...
- Frame all copy recommendations around capital deployment, EU residency access, and family safety — never around visa paperwork or eligibility filtering.
...
- Investor migration demand is correlated with geopolitical instability, currency devaluation, and tax policy changes in source markets.
- When analyzing traffic spikes or drops, check for macro events in source countries before attributing changes to campaign performance.
- Geographically segmented analysis is high-value — the same campaign can perform very differently across source markets depending on current events. Always slice by geography when diagnosing anomalies.
...
The correct frame: you have capital → we help you invest it → consultation is the next step. The CTA is always to start a conversation, never to filter leads. Use **"Request a Free Consultation"** — no alternatives.
...
- Robots.txt
- Organic traffic (zero organic is correct and expected)
- Sitemaps
- Any organic/SEO metric of any kind

Raising these is factually incorrect in this context and wastes the account owner's time. Remove SEO from all CRO audits on this domain.
...
- **Do not diagnose a landing page's design before its copy.** Visual design is strong. Copy is the weak link in this account.
...
- **Conversion tracking**: All campaigns → inline gtag() + GTM (GTM-K6864NBH). Primary conversion: GV Lead (`fc6FCO3YnI4cELCTg4oD`). Dual-layer infrastructure confirmed working. Do not add per-campaign tags. Do not audit tracking.
...
- **CTA standard**: "Request a Free Consultation" — applies account-wide on all campaigns and all copy recommendations. No alternatives.
- **Attribution window**: High-ticket investment product. Expect multi-day conversion lag. Recent data (< 7 days) will always undercount conversions. Do not act on recent data without checking lag first.
...
- **Copy gap**: "Plan B" / family safety / geopolitical hedge messaging is underutilized and is the highest-value CRO recommendation for any copy or landing page audit.
