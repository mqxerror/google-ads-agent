...
class BudgetService:
    ...
    async def remove_campaign_budget(
        ...
        name), same as campaigns — Google rejects status updates to
        REMOVED. Only succeeds when no non-removed campaign references
        the budget (remove the campaign first; used by the PMax
        ...
        try:
            ...
            await ctx.log(
                level="info",
                message=f"Removed campaign budget {budget_id} for customer {customer_id}",
            )
